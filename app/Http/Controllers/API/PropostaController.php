...
class PropostaController extends Controller
{
    ...
    public function index()
    {
        $rows = DB::table('propostas')
            ...
            ->where('propostas.tenant_id', $this->tenantId())
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($data) {
            ...
            DB::table('propostas')->where('tenant_id', $this->tenantId())->where('id', $id)->update(['valor_total' => $total, 'updated_at' => now()]);

            return response()->json(DB::table('propostas')->where('tenant_id', $this->tenantId())->where('id', $id)->first(), 201);
        });
    }

    public function show(int $id)
    {
        $proposta = DB::table('propostas')->where('tenant_id', $this->tenantId())->where('id', $id)->first();
        $linhas = DB::table('proposta_linhas')
            ...
            ->where('proposta_linhas.tenant_id', $this->tenantId())
    ...
    public function toOrder(int $id)
    {
        return DB::transaction(function () use ($id) {
            ...
            $total = 0;
            $lines = DB::table('proposta_linhas')->where('tenant_id', $this->tenantId())->where('proposta_id', $id)->get();
            ...
            DB::table('encomendas_clientes')->where('tenant_id', $this->tenantId())->where('id', $orderId)->update(['valor_total' => $total, 'updated_at' => now()]);
    ...
    {
        $count = DB::table($table)->where('tenant_id', $this->tenantId())->count() + 1;
