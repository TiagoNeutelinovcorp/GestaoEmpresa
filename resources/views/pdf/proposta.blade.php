...
<body>
    <div class="header">
        <p class="title">Proposta</p>
        <p class="subtitle">Documento {{ $proposta->numero }}</p>
    </div>

    <div class="meta">
        <p><strong>Número:</strong> {{ $proposta->numero }}</p>
        <p><strong>Data:</strong> {{ $proposta->data_proposta }}</p>
        <p><strong>Validade:</strong> {{ $proposta->validade }}</p>
        <p><strong>Cliente:</strong> {{ $proposta->cliente_nome ?? '-' }}</p>
        <p><strong>Estado:</strong> {{ ucfirst($proposta->estado ?? 'rascunho') }}</p>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @if($linhas->isEmpty())
                <tr>
                    <td colspan="6">Sem linhas de artigo.</td>
                </tr>
            @endif
        </tbody>
    </table>
    ...
    <div class="total-box">
        Total: {{ number_format($proposta->valor_total, 2, ',', '.') }} €
    </div>
</body>
</html>
